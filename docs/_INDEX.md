...
| `STUDIO_CLOSURES.md` | ✅ *(Draft v2)* | Studio Closures | studio_closures (`all_studios`, `closed_through`, `is_total`, `exempt_event_types` — new), closure_locations (new), private_sessions (`location_id`, `overrides_closure` — new), classes (`closure_exempt` — new), schedule_instances, timesheet_entries | admin/settings/studio-calendar, `apply_closures` (new), `generate_occurrences` | **v2 2026-07-29 merges the 2026-07-10 location spec (`fe53713`) and the 2026-07-29 rewrite (`636ba43`) — neither superseded the other.** Closures become a **cancellation pass**, not a generation filter: `apply_closures(p_dry_run default true)` cancels occurrences under a closure at any time, which is both the last-minute-closure path and the missing un-generate path. Location scoping via `all_studios` + `closure_locations` join (studio-type locations only — never partner_venue/internal); date ranges via `closed_through`; total-vs-partial via `is_total`; exemptions via `exempt_event_types` + `classes.closure_exempt`. **Override precedence ladder (§6):** `is_total` defeats *every* exemption and override, including a teacher's `overrides_closure` and an admin's class override; teachers may override a private (no admin gate), only admin/super_admin may override a class. Deletes **draft** timesheet entries for cancelled occurrences, never approved/submitted/paid — those are reported by teacher+date for a human (§9). **Class-side no longer deferred** — the generator shipped 2026-07-29. **Blocked piece is now privates enforcement**, pending `private_sessions.location_id` (location is free-text `"Studio 1"` on all 5 rows today) |
