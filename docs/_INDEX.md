...
| `CLASS_OPERATIONS_MODE.md` | 🧱 *(Draft v1, 2026-07-30 — awaiting approval, not built)* | Class Operations / Attendance | all new/unbuilt: attendance **snapshot** table⚠ (one row per occurrence, keyed `schedule_instance_id`, written once), append-only **change** table⚠ (`late_arrival`/`correction`/`early_departure`), **reason picklist** table⚠ (`refunds.reason_id` pattern — FK, not free text), tenant config on `studio_settings` *or* a dedicated `class_operations_config`⚠ (§3, §5); reads `schedule_instances`, `classes`, `profile_roles` | **none built** — proposed: teacher attendance UI w/ countdown (§7), ~admin escalation/resolution queue (§6) | Attendance snapshot **locks N minutes after class start** — tenant-configurable, **OFF by default**. The lock is on the *initial roster snapshot*, not the attendance record: after close, late arrivals and corrections are **appends** — attributed, timestamped, **no escalation** — and **only a MISSING snapshot escalates** (§2, §6.1); a snapshot taken at minute 4 and corrected six times at minute 40 is fully compliant. Admin may **reopen** a window; overrides and reopenings require a reason from a tenant-configurable **picklist** (`refunds.reason_id` pattern), `forgot` deliberately on the list. **Two separate metrics, never blended** (§8.1): **completion** (was it taken at all — a reopened snapshot **counts**) and **timeliness** (was it taken inside the window — a reopened snapshot **does not**). Grading is **emitted, not owned** (§8.0) — a separate teacher-recognition module will also consume **curriculum adherence** and **substitution stars**, so this spec publishes two facts per occurrence and does not own the scoreboard. Never grades roster *content* (§8.2); does not gate pay (§6.4). 🔴 **Two blocking prerequisites, both wider than this spec:** (1) **`markAttendance` authorizes on `classes.teacher_id`, so a substitute cannot mark attendance at all** — under a window every substituted class fails and escalates *automatically*, filling the queue with an authorization bug rather than compliance failures; authorization must also read `schedule_instances.substitute_teacher_id` (§4.1). (2) **The `studio_manager` role does not exist — 0 rows** — and `STUDIO_CLOSURES.md` references it too, so neither spec can enforce it until the role is created (§4.2) |
...
| `COMMERCE_AND_BILLING.md` | ✅ *(Draft)* | Commerce & Billing | ledger_entries (new), enrollments, class_pricing_rules, bundle_configs, enrollment_carts | api/enroll, api/enrollment, api/webhooks/stripe | Master financial backbone: one dimensioned ledger, reconciled checkout spine, review-and-approve billing, expenses/QBO, pricing/bundle/discount engine, Angelina upsell. Not blocked by task 19; `allow_single_day` (CLASS_MEETINGS) is a pricing input |
...
| `OCCURRENCE_GENERATION.md` | ✅ | Scheduling | schedule_instances, classes (`days_of_week`, `start_date`, `end_date`, `is_rehearsal`, `trial_eligible`), studio_closures, tenants, timesheet_entries | `generate_occurrences` (new 2026-07-29), migrations 20260729000001-3, api/teach/roster | **Canonical for how class occurrences are materialized.** Task 19. Per-class `[start_date, end_date]` window (the season table is not consulted); ISO weekdays **Monday=1..Sunday=7** pinned by CHECK — `0` is rejected because `isodow`/`dow` diverge only on Sunday and would silently generate nothing for the first Sunday class; idempotent via a **partial** unique index on `(class_id, event_date, start_time) WHERE class_id IS NOT NULL` + bare `ON CONFLICT DO NOTHING`; **never touches the past** (`event_date > current_date`, no UPDATE, no DELETE) — that is what makes the occurrence a durable teacher snapshot, so `class_teachers` needs no effective dating. **Refuses to run multi-tenant** (`classes` has no `tenant_id`; guard raises when `tenants > 1`). **Phases 0–3 shipped 2026-07-29:** constraints (`2633d1a`), the function (`3222fec`), 61 orphan occurrences cancelled not deleted (`d7e7b9f`). **Phase 4 (first Fall generation) blocked on the Fall closure calendar** — only 6 closure rows exist, all Spring Break; generating against an incomplete list produces occurrences on days the studio is shut, which then produce timesheet drafts and proration against dates nobody worked. `STUDIO_CLOSURES.md` v2 Phase 3 removes closure handling from this function (breaking signature change: drops `dates_skipped_closed`) and unblocks Phase 4 by making closures undoable |
...
| `STUDIO_CLOSURES.md` | ✅ *(Draft v2)* | Studio Closures | studio_closures (`all_studios`, `closed_through`, `is_total`, `exempt_event_types` — new), closure_locations (new), private_sessions (`location_id`, `overrides_closure` — new), schedule_instances, timesheet_entries | admin/settings/studio-calendar, `apply_closures` (new), `generate_occurrences` | **v2 2026-07-29 merges the 2026-07-10 location spec (`fe53713`) and the 2026-07-29 rewrite (`636ba43`) — neither superseded the other.** Closures become a **cancellation pass**, not a generation filter: `apply_closures(p_dry_run default true)` cancels occurrences under a closure at any time, which is both the last-minute-closure path and the missing un-generate path. Location scoping via `all_studios` + `closure_locations` join (studio-type locations only — never partner_venue/internal); date ranges via `closed_through`; total-vs-partial via `is_total`; exemptions via `exempt_event_types` on the closure (confirmed `{private_lesson}` — no rehearsal exemption) **plus a per-occurrence admin override** (§6 rung 4, admin/super_admin only). **`classes.closure_exempt` is withdrawn (2026-07-30, D1)** — never created, retracted not deferred; Pilates is a private lesson and adult classes take the per-occurrence override. **Override precedence ladder (§6):** `is_total` defeats *every* exemption and override, including a teacher's `overrides_closure` and an admin's class override; teachers may override a private (no admin gate), only admin/super_admin may override a class. Deletes **draft** timesheet entries for cancelled occurrences, never approved/submitted/paid — those are reported by teacher+date for a human (§9). **Class-side no longer deferred** — the generator shipped 2026-07-29. **Blocked piece is now privates enforcement**, pending `private_sessions.location_id` (location is free-text `"Studio 1"` on all 5 rows today). **Amended 2026-07-30:** adds **§15 — the 2026–2027 closure calendar, 12 rows** (Amanda-confirmed: studio **is** closed Veterans Day, **not** closed Lincoln Day, which is removed), entered as ranges; plus §15.1 weekday impact. This spec is now the source for which dates the studio is closed next season, not just the mechanism |
...
19. **🔴 P0 — Schedule occurrences: two competing models. `class_sessions` does not exist at all; `schedule_instances` is live and now has a working generator.** Investigated 2026-07-09; the generator half **resolved 2026-07-29** (`OCCURRENCE_GENERATION.md`, migrations `20260729000001-3`). Still P0 because the entire `lib/schedule/` stack is written against the phantom table and fails silently — that half is unchanged.
    ...
    - **`schedule_instances` is the live table and now HAS a generator.** ~~Only 61 rows, all 2026-03-09→03-14 (one stale week)… nothing populates current occurrences.~~ **Corrected 2026-07-29, verified against the live DB:** `generate_occurrences` shipped (`3222fec`) and `schedule_instances` holds **1,042 rows — 981 published spanning 2026-07-23 → 2026-11-26, plus the 61 March rows now `status='cancelled'`** as orphaned occurrences with no `class_id` (`d7e7b9f` — cancelled, not deleted, because six FK columns across five tables reference this table). The 61-row figure in the 2026-07-09 investigation was the March seed week specifically, not the whole table. Still true: **0 `schedule_templates`**, `template_id`/`created_by` all null, and the legacy `generateSessionsForClass` targets the phantom `class_sessions` — it is not the generator that shipped and remains dead code. **Coverage is still partial: 66 of 153 active classes have occurrences.** Phase 4 (first full Fall generation, ~2,918 rows across 91 classes in a rolled-back test) is blocked on the Fall closure calendar — see `OCCURRENCE_GENERATION.md` §8 Q1 and `STUDIO_CLOSURES.md` v2.
    ...
    - **Correct instance surface requires an occurrence generator FIRST**, then repointing `class_sessions`→`schedule_instances` with column renames (`session_date`→`event_date`, `lead_teacher_id`→`teacher_id`, `session_notes`→`notes`, `room` text→`room_id`, `is_cancelled`→`status='cancelled'`) and a schema decision for the no-equivalent columns (`attendance_locked_at`, `needs_coverage`, `duration_minutes`, `cancellation_pay_decision`/`_rate_override`, `timesheet_entries_generated`, …). This is the root blocker for LOCATIONS Step 4b (see `LOCATIONS_AND_FACILITIES.md` §10). Extends the naming-drift item (15) with the full footprint. **No longer gates `STUDIO_CLOSURES.md` — resolved 2026-07-29.** The generator shipped (`OCCURRENCE_GENERATION.md`, migrations `20260729000001-3`; commits `2633d1a` / `3222fec` / `d7e7b9f`), so class occurrences now exist to cancel and the class-side of that spec is buildable. `STUDIO_CLOSURES.md` **v2** reframes closures as a **cancellation pass** (`apply_closures`, dry-run by default) rather than a generation-time filter, with location scoping (`all_studios` + `closure_locations`), ranges (`closed_through`), total-vs-partial closures (`is_total`), exemptions (`exempt_event_types` on the closure — confirmed `{private_lesson}` — plus a per-occurrence admin override, §6 rung 4, admin/super_admin only; the proposed `classes.closure_exempt` was **withdrawn 2026-07-30**, never created), and an override precedence ladder in which `is_total` defeats every override including a teacher's private override and an admin's class override. **The blocked piece is now the privates side**, not the class side: `private_sessions.location_id` does not exist and location is still free-text (`"Studio 1"` on all 5 rows), so no location-scoped enforcement works on privates until that column and its backfill land. That spec's Phase 3 also **removes closure handling from `generate_occurrences`** — a breaking change to a published contract (drops the `dates_skipped_closed` return column), cheap now because nothing calls the function and no cron depends on it.
    - **Feeds from `class_meetings` (new — see `CLASS_MEETINGS.md`, Draft 2026-07-10).** The generator must materialize `schedule_instances` from **`class_meetings`** (per-meeting `day_of_week`/`start_time`/`end_time`/`room_id`), **not** the deprecated `classes` scalar day/time — design it against `class_meetings` from the start (that table is scoped precisely to give the generator per-meeting day/time/room). **Billing-engine dependency:** `CLASS_MEETINGS.md` also adds `classes.allow_single_day` (the commitment toggle) — stored + displayed now, but the billing engine (`COMMERCE_AND_BILLING.md` §8, pricing/bundle engine) reads it to drive per-day vs. whole-class pricing; keep the column's meaning stable and do not repurpose it.
    - **NOT a blocker for payment/enrollment (see `COMMERCE_AND_BILLING.md` §1, §3).** Charge creation and enrollment-record writing are independent of the occurrence stack — the reconciled checkout spine (Layer 1) can be built and tested end-to-end without a generator. Only the post-enrollment **"upcoming sessions"** display (`/portal/students`) depends on task 19, so a newly enrolled family sees an empty schedule until the generator lands.
