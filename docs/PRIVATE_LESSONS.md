...
## 16. Policy Decisions (Resolved)

### Cancellation Policy — Configurable Per Teacher
Each teacher has their own cancellation policy set in their rate card.
Default: 24-hour notice required for full refund. Late cancellation
(under 24 hours) charges the full session rate.

Admin sets per teacher in Admin → Teachers → [Name] → Rate Card:
- `cancellation_notice_hours` — hours of notice required (default: 24)
- `late_cancel_charge_pct` — % of session rate charged on late cancel (default: 100%)
- `no_show_charge_pct` — % charged for no-show (default: 100%)

```sql
-- Additional fields on teacher_rate_cards:
cancellation_notice_hours   INTEGER NOT NULL DEFAULT 24,
late_cancel_charge_pct      NUMERIC(5,2) NOT NULL DEFAULT 100.00,
no_show_charge_pct          NUMERIC(5,2) NOT NULL DEFAULT 100.00,
cancellation_policy_note    TEXT,   -- shown to parents at booking
```

### Co-Teacher $0 Sessions — Always Timesheet-Tracked
When Amanda or any teacher teaches a session at $0 (gifted), they still
receive a timesheet entry at $0. This ensures:
- Amanda's contribution is tracked in the studio contribution report
- The full value of gifted sessions is visible in Finance reporting
- Teachers cannot be accidentally excluded from timesheet history

### Parent Booking Visibility — Admin Approval Required First
Parents cannot see a teacher's open availability until admin explicitly
grants booking approval for that family/student + teacher combination.
No "browse available teachers" mode exists. Availability is only revealed
after admin has approved the relationship.

### Hybrid Sessions — Flat Rate
Hybrid sessions (e.g. 30 min ballet + 30 min Pilates) are billed at a
single flat rate set by the teacher at session creation. No per-segment
point cost calculation. A `session_notes` field captures the breakdown
(e.g. "30 min technique / 30 min Pilates") for the teacher's and parent's
reference but does not affect billing.

### Studio Contribution Display — Show When > $0
The studio contribution display is shown to parents whenever the
contribution is greater than $0. This requires Super Admin (Studio Owner)
to have the feature enabled in Admin → Settings → Billing → Show studio
contributions to parents. Default: enabled.
The display is always accurate — it is never shown for a $0 contribution.
