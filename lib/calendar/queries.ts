...
export async function getScheduleInstances(
  ...
  const classInfo: Record<string, { name: string; levels: string[] | null; style: string | null; age_min: number | null; age_max: number | null; discipline: string | null }> = {};
  if (classIds.length > 0) {
    ...
    for (const c of classes ?? []) {
      classInfo[c.id] = { name: c.name, levels: c.levels, style: c.style, age_min: c.age_min, age_max: c.age_max, discipline: c.discipline };
    }
  }
